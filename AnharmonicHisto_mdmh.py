...
u = MDAnalysis.Universe('/home/xov/mdmx/top_dcd/MDMX_mdm.pdb', '/home/xov/mdmx/top_dcd/mdmh_MDMX.dcd', permissive=False);
...
tmp = numpy.reshape(numpy.tile(avgCoords, 50000), (50000,dim*Na)).T; #changed 10000 to 50000
...
plt.savefig(filename='mdmh_QAA_fig1');
plt.show();
...
plt.savefig(filename='mdmh_QAA_fig2');
plt.show();
...
ax.plot(y);
plt.savefig(filename='mdmh_QAA_fig3');
plt.show();
...
plt.savefig(filename='mdmh_QAA_fig4');
plt.show();

# some set up for running JADE
Ncyc  = 1;
subspace = 50; #50 covers ~95 percent variance. 
...
plt.savefig(filename='mdmh_QAA_fig5');
plt.show();
